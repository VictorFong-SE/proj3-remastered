...
namespace Catch {
    ...
    namespace Detail {
        ...
        std::string clrReferenceToString( Type^ ref ) {
            if (ref == nullptr)
...
#if defined(_MANAGED)
        template <typename Type>
        std::string stringify( Type^ e ) {
            return ::Catch::StringMaker<Type^>::convert(e);
        }
#endif
...
# endif
    ...
    struct StringMaker<Type^> {
        static std::string convert( Type^ ref ) {
...
#pragma warning(disable:4312) // Converting int to Type* using reinterpret_cast (issue on x64 platform)
...
public:
    ...
    // infering std::function<bool(Type const&)> is hard (but possible) and
    ...
    namespace Vector {
        ...
        struct EqualsMatcher : MatcherBase<std::vector<T>> {
            ...
            bool match(std::vector<T> const &v) const override {
                ...
                // to using !=. but could be specialised for, e.g. std::vector<Type> etc
...
namespace detail {
    ...
    template<typename Type>
    inline auto convertInto( std::string const &source, CLARA_CONFIG_OPTIONAL_TYPE<Type>& target ) -> ParserResult {
        Type temp;
